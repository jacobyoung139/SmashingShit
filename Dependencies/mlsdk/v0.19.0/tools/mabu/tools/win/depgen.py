...
def main():
    assert platform.system() == "Windows", "The include dependency generator can only be used on Windows!"
    assert "VCINSTALLDIR" in os.environ, "VCINSTALLDIR not found in the environment! The include dependency generator should either be called from Mabu or after running vcvarsall.bat."

    desc = "Resolve the include dependency hierarchy of a given input source file and convert it to a format compatible with GNU make. This should only be run from Mabu!"
    ...
    parser.add_argument("-i", "--include", action="append", default=[], help="Supplemental include path to search.")
    parser.add_argument("-d", "--define", action="append", default=[], help="Macro to pass to the preprocessor.")

    opt = parser.parse_args()
    temp_out_dir = tempfile.mkdtemp()
    tool = "cl.exe"
    inc_args = [os.path.abspath(i) for i in opt.include]
    def_args = opt.define
    ...
    + ["/I{}".format(i) for i in inc_args] \
    + ["/D{}".format(d) for d in def_args]
