...
from diags import warn, error, debug, info
...
class ProjectFactory(object):
        ...
        for scope in [PRIVATE_SUFFIX, '']:
            ...
            if outname:
                if len(outname) == 0:
                    ...
                    defs[varname] = outname[0]
                else:
                    defs[varname] = outname[0]

            # OUT is not a list and needs to be applied directly
            if self._ext != COMP_EXT:
                varname = OUT + scope
                out = defs.get(varname)
                if out is not None:
                    if len(out) > 1:
                        self.file_error("only one string expected for OUT, got: {0}".format(out))
                    self._project.output_directory = out[0]
    ...
    def _fetch_uses(self, defs, project):
        ...
        for comp in all_uses:
            ...
            if comp in build_env.components():
                ...
                path = os.path.realpath(os.path.join(project.base, comp))
                if not os.path.isfile(path):
                    path = None
                    for base in build_env.search_path():
                ...
                if not path:
                    self.file_error("cannot locate component '{1}' referenced in USES of "
                                    "project {0};\n{2}"
                                    .format(project.name, comp, build_env.format_search_path_error(project.base)))
                    continue
    ...
    def validate(self):
        ...
        if not self._ext in extension_to_vars:
            # will fail elsewhere
            return
    ...
    def _process_src_extensions(self, lst):
        ...
        while i < length:
            ...
            tuples.append((Path(self.project().base_path(), mk), src_ext, dst_ext, Path.parse(dst_path)))
...
def load_project(path, name=None, text=None,
                 output=None,
                 validate=True, register=True, builtin=True):
    """
...
def load_project_and_dependencies(path, outdirs, base=None, name=None, set_options=None):
    ...
    for wrn in factory.warnings():
        warn(wrn[2], wrn[0], wrn[1])

    any = False
    for err in factory.errors():
        error(err[2], err[0], err[1], die=False)
        any = True

    proj_ext = factory.extension
    if any:
        error("one or more {0} failed to load"
              .format(_ext_labels.get(proj_ext)))
