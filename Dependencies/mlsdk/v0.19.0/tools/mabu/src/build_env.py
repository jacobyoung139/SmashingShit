...
from config import DATA_DIR, CONFIG_EXT, OPTION_EXT, COMP_EXT
...
def format_search_path_error(proj_base=None):
    """
...
def setup_mabupath(user_projects, external_mabu):
    """
    ...
    # get project paths
    for proj_path in user_projects:
...
def _register_project_directory(path, base='', builtin=True, exts=[OPTION_EXT, CONFIG_EXT], builtin_roots=[]):
    ...
    if '.metadata' in ents:
        # looks like we're in a nested MLSDK directory: don't enter (for e.g. `tests/data/alt_mlsdk`)
        return
    ...
    for ent in ents:
        ...
        if os.path.isfile(full):
            ...
            if ext in exts:
                ...
                load_project(full, name=proj_name, builtin=builtin)
        else:
