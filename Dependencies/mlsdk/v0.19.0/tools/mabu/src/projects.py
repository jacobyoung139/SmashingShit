...
from utils import unix_path, remove_duplicates, path_to_filename
...
class Project(object):
    ...
    def __init__(self, path, name, base, output=None):
        ...
        # map of input source to generated source
        self._gen_src_map = {}
        """ :type: dict[Path,Path] """
    ...
    @user_output.setter
    def user_output(self, output):
        self._user_output = output
        self.update_output_dirs()
    ...
    def update_output_dirs(self):
        ...
        self._outdir_path_var = Variable(self.name_var() + '_' + build_vars.OUTPUT, out_dir)
    ...
    def output_name(self):
        """
        Get the name of the project output.
        :return: Returns OUTNAME if defined, self._name otherwise.
        :rtype: str
        """
        varname = self.is_package() and OUTNAME or OUTNAME + PRIVATE_SUFFIX
        outname = self.defs.get(varname, None)
        return outname if outname else self._name
    ...
    def get_all_external_dependencies(self):
        ...
        :rtype: list[build_model.Path or build_model.Composition]
        """
        components = [x.path for x in self.dependency_list(None, build_env.components())]
        extra_dependencies = sorted(set(components + self.options + self.configs))
    ...
    def all_source_files(self):
        """ Generate list of the source files used by the project --
    ...
    def _gather_wildcard_datas(proj, src_path, target_path, src_absolute):
        ...
        src_glob = src_path.resolve().replace('\\', '/')
        ...
        brack_pos = pos_or_end('[')
        wild_pos = min(star_pos, quest_pos, brack_pos)

        slash = src_glob.rfind('/', 0, wild_pos)
        src_glob_prefix = src_glob[0:slash + 1]

        ents = []

        for ent in glob.glob(src_glob, recursive=True):
            ...
            target_suffix = ent[len(src_glob_prefix):]
    ...
    def all_data_files(self, scope=0, clean=False):
        ...
        :return: list of of input full paths and output package paths, list of errors
        :rtype: list[tuple[Path, Path]], list[Str]
        """
        errors = []
        ...
        def data_paths(proj, src_target):
            ...
            if is_wild_source(src):
                ...
                if not ents:
                    diags.warn('no entries matched wildcard in DATAS ({0} : {1})'.format(
        ...
        return remove_duplicates(datas), errors
    ...
    def generated_sources(self):
        """
        :rtype: dict[build_model.Path, build_model.Path]
        """
        return self._gen_src_map
    ...
    def all_outputs(self):
        ...
        if not self.is_component():
            ...
            outs += self._gen_src_map.values()

        # layout's files
        out_files, _ = self.all_data_files()
    ...
    def get_package_mappings(self, force_package, scope):
        ...
        :param scope: 0 for all, -1 for local, 1 for refs
        :return: two lists of tuples (for input to output files, for builds and data mappings), and errors
        :rtype: tuple[list[tuple[str,str]], list[tuple[str,str]]], list[str]
        """
        env = build_env
        ...
        # actual DATAS
        datas, sub_errors = self.all_data_files(scope)
        errors += sub_errors
        ...
        return build_mapping, data_mapping, errors
    ...
    def resolve_settings(self, config):
        ...
        for key, val in self._defs.items():
            if key.endswith(PRIVATE_SUFFIX):
                key = key[:-len(PRIVATE_SUFFIX)]
            elif key not in ALWAYS_EXPORTED_VARS:
                continue
        ...
        if OUT in self.combined:
            ...
            if outdirs:
                ...
                # so use the last one
                self.output_directory = outdirs[-1]
        else:
    ...
    def _expand_src_extensions(self):
        ...
        ext_map = {}
        for (_, src, dst, pfx) in self.combined[SRC_EXTENSIONS]:
            ext_map[src] = (dst, pfx)
        ...
        for project, src in self.combined[SRCS]:
            ...
            if ext in ext_map:
                ...
                if project:
                    self._gen_src_map[Path(project.base_path(), src)] = \
                        Path(project.base_path(), cvt_path)
                else:
                    self._gen_src_map[src] = cvt_path
