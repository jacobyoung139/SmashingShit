...
import build_env
from build_model import Variable, Buildable, Path, Command, Rule, EnvironmentVariable, InfoCommand, Include, VariableDefinition, \
    Text, Node, Directive, split_by_length
...
from run_vars import SPEC, HOST, runtime_values
...
class MakeGen(object):
    ...
    def _generate_one(self, proj, rules, ext_dependency_projects):
        ...
        prebuild_rule = Rule(None, Buildable(proj.name_var() + '-prebuild'), [])
        prebuild_rule.multi_target = True
        rules.append(prebuild_rule)

        postbuild_rule = Rule(None, Buildable(proj.name_var() + '-postbuild'), [])
        postbuild_rule.multi_target = True
        rules.append(postbuild_rule)

        rules.append(self._create_clean_rule(proj))

        project_deps = ext_proj_deps + \
                       [ prebuild_rule.buildable ] + \
                       orig_targets + \
                       [ postbuild_rule.buildable ]
        # ensure the dependencies are built in this strict order
        ...
        # add prebuild/postbuild/extension support
        rules += self._create_ext_rules(proj)

        debug(str(rules))
        mks = Path(proj.outdir_path(), proj.name + ".mk")
        self._mkfiles.append((proj.name, (mks, rules)))

    def _create_ext_rules(self, proj):
        ...
        saw_mks = set()
        for path in proj.all_paths(EXTENSIONS):
            resolved = path.resolve()
            if resolved not in saw_mks:
                saw_mks.add(resolved)
                rules.append(Include('include', path))

        for (path, _, _, _) in proj.combined[SRC_EXTENSIONS]:
            resolved = path.resolve()
            if resolved not in saw_mks:
                saw_mks.add(resolved)
                rules.append(Include('include', path))
    ...
    def _create_clean_rule(proj):
        ...
        clean_rule = Rule(None, Buildable(proj.name_var() + '-clean'), [])
    ...
    def gather_vars(self):
        ...
        for proj, (_, directives_rules) in self._mkfiles:
            vars = self._gather_vars(directives_rules)
            var_map[proj] = vars
    ...
    def _gather_vars(self, rules):
        ...
        if SPEC not in vars:
            vars[SPEC] = VariableDefinition(SPEC, '=', str(build_env.spec()))

        if HOST not in vars:
            vars[HOST] = VariableDefinition(HOST, '=', runtime_values[HOST])
