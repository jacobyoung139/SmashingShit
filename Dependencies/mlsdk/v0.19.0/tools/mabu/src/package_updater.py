    ...
    * to inject <application ml:min_api_level> attribute
    ...
    def _configure_developer_id(self, pkgflags):
        ...
        try:
            ...
            if pkgflags.manifest.developer_id:
...
class PackageEntry(object):
    ...
    @staticmethod
    def _file_state(path, size, hash):
        """
    ...
    def set_transforms(self, pkgflags):
        """
        ...
        if self._xfrms:
            if self._xfrmed_path != self._local_path:
                ...
                xfrm_path = os.path.join(
                    ...
                    path_to_filename(self._local_path))
...
class PackageUpdateScheduler(object):
    ...
    def __init__(self, build_mapping, data_mapping, pkgflags, output_file, cache_file):
        ...
        # read the current zip state -- only validating the existence of files
        # (we assume the user did not muck with the file except perhaps to
        # delete a file)
        try:
            with zipfile.ZipFile(self._output_file, 'r') as zf:
                self._zip_files = zf.namelist()
        except OSError:
            self._recreate = True
            self._zip_files = []
    ...
    def process_archive_candidate(self, local_src, arcname, clean,
    ...
    def generate(self, clean=False):
        ...
        diags.debug("Considering delete steps: saw entries: {};\n"
                    ...
                    .format(entries_keys_seen, known_arcnames))
        ...
        if not clean:
            ...
            unknown_arcnames = set(self._zip_files)
        ...
        def func_order(step):
            if isinstance(step, RemoveArchiveFileStep):
                return 1
            if isinstance(step, BaseWriteArchiveFileStep):
                return 2
            return 0  # e.g. transform, delete, etc.

        steps.sort(key=func_order)
