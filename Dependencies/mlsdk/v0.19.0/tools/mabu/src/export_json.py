...
def export_json(scope, output_file_path, explicit_projects):
    ...
    def get_value_for_project_def(def_name, def_value, proj):
        ...
        def get_component_dict():
            ...
            for comp in def_value:
                ...
                if comp in env.components():
                    ...
                    for search_path in env.search_path():
                        comp_file_full_path = os.path.abspath(os.path.join(search_path, comp_file))
                        if os.access(comp_file_full_path, os.F_OK):
        ...
        elif def_name in ["OUT", "MLCERT", "SIGNER"]:
            return def_value[0] if def_value else None
        elif def_name in ["OUTNAME"]:
            return def_value if def_value else None
    ...
    for proj in all_projects:
        ...
        if proj.is_program():
            output_filename = build_platform.format_program_file(proj.output_name)
        elif proj.is_shared():
            output_filename = build_platform.format_shared_file(proj.output_name)
        elif proj.is_static():
            output_filename = build_platform.format_static_file(proj.output_name)
        else:
            continue
