...
class GCCLike(Toolchain):
    ...
    def __init__(self, name, version=None, platform=None):
        Toolchain.__init__(self, name, version, platform)
    ...
    def get_unique_artifacts(self, proj):
        ...
        if build_env.spec().platform == platforms.lumin.NAME:
    ...
    def create_compiler_commands(self, env, proj, kind, source_path, obj_path):
        ...
        args = [
            ...
            is_cxx and self.tool_var('CXX') or self.tool_var('CC'),
    ...
    def _gather_libs(self, model, libpaths):
        ...
        # The library grouping is only needed for certain platforms.
        requires_lib_group = (model.stlibs or model.shlibs) \
            and platform.name != platforms.osx.NAME

        dirs = []
        libs = []

        if requires_lib_group:
            libs += ["-Wl,--no-as-needed", "-Wl,--start-group"]
        ...
        if build_env.spec().platform == platforms.win.NAME:
            ...
            type_args.append(Composition(
                ...
                Path(proj.outdir_path(),
                     self.format_importlib_file(proj.output_name))))
    ...
    def add_strip_commands(self, commands, out_path):
        ...
        if build_env.spec().platform == platforms.lumin.NAME:
            ...
            commands += [
                Command([self.tool_var("OBJCOPY"), "--only-keep-debug", out_path, sym_file]),
                Command([self.tool_var("OBJCOPY"), "--add-gnu-debuglink", "{}".format(sym_file), out_path, out_path]),
                Command([self.tool_var("STRIP"), "--strip-unneeded", out_path])
            ]
    ...
    def add_cross_toolchain_tool(self, var, tool):
        """
        ...
        arch = arch_default(self._platform)
        root_name = arch_map[arch] + '-linux-android-'

        mltools = run_vars.runtime_values.get(run_vars.MLTOOLS)
        binDir = os.path.join(mltools or '', 'bin')

        toolPath = os.path.join(binDir, root_name + tool)
        if not os.path.exists(toolPath):
            if HOSTOS == OS_WIN:
                toolPath += ".exe"
            if not os.path.exists(toolPath):
                return "Tool {0} does not exist".format(toolPath)

        # keep paths in Unix-friendly format
        toolPath = os.path.normpath(toolPath).replace('\\', '/')

        self._tools[var] = toolPath
