...
from config import NAME, PACKAGE_EXT, OS_LINUX, OS_OSX, get_dev_sdk_path, BUILT_PACKAGE_EXT
...
def parse_arguments():
    ...
    build_group.add_argument("-v", "--verbose", dest="verbose", action='store_true',
                             help="verbose build")
    build_group.add_argument("-j", "--jobs", dest="jobs", metavar='N', nargs='?',
    ...
    print_group.add_argument("--print-invoke-commands", dest="print_invoke_commands",
                             help="print the commands needed for invoking the project output", action="store_true")
    ...
    invoke_group.add_argument("--invoke", dest="invoke",
                              help="invoke the project output after building", action="store_true")
    invoke_group.add_argument("--no-mlremote-adjust", dest="no_mlremote_adjust",
                              help="with --invoke, turns off behavior to avoid letting "
                                   "mabu LIBPATHS override any ML Remote entries in the current runtime library path",
                              action="store_true")
    invoke_group.add_argument("--invoke-args", dest="invoke_args",
                              help="argument to pass to the output of --invoke or --print-invoke-config", action="store")
    ...
    special_group.add_argument("--re-sign", "--resign", dest="resign_mpk", metavar='INPUT_MPK',
                               ...
                               "writing a copy with the alternate signatures applied".format(ext=BUILT_PACKAGE_EXT))
...
def _validate_options(opts):
    ...
    if opts.resign_mpk:
        ...
        if not opts.mlcert:
            error("--re-sign requires the -s/--signer argument")
...
def add_convenience_variables():
    ...
    runtime_values[HOST] = os_platform_map[HOSTOS]
