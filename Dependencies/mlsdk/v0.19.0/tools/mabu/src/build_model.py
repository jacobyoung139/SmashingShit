...
class Composition(Node):
    ...
    def __repr__(self):
        return "".join([repr(s) for s in self._segments])
...
class Path(Composition):
    ...
    def __init__(self, *segments, sep='/'):
        ...
        Composition.__init__(self, *new_segs)
        self._sep = sep
...
class Command(Composition):
    ...
    def __repr__(self):
        cmd_str = " ".join([str(x) for x in self._segments])
        return cmd_str
    ...
    def makefile_str(self):
        ...
        return " ".join([wrap(x) for x in segs])
...
class VariableDefinition(Directive):
    ...
    def makefile_str(self):
        ...
        return self.name().value() + self.op().value() + var_str
...
class BuildSpec(object):
    ...
    """
    def __init__(self, spec=None, no_defaults=False, ignore_missing=False):
        """
                    ...
                    if Toolchain.find(seg[0:idx]):
                        if ignore_missing:
                            ...
                            raise BuildException(
                                "toolchain '{1}' is not registered (in build specification '{0}'); "
                                "instead, try:\n{2}".format(spec, seg,
    ...
    def __repr__(self):
        return '_'.join([str(x) for x in [self._config, self._target, self._toolchain, self._arch]])
    ...
    def is_compatible(self, spec):
        ...
        This assumes the receiver is generic and @spec is specific, e.g.
        `lumin` vs. `debug_lumin_gcc-4.9_aarch64`.
        :rtype: bool
        """
        if self._toolchain and spec.toolchain:
            if self._toolchain != spec.toolchain:
                # toolchain names may be versioned
                self_and_vers = self._toolchain.split('-')
                ...
                if len(self_and_vers) > 1 and len(other_and_vers) > 1:
                    if self_and_vers[1] != other_and_vers[1]:
                        self_vers_major = self_and_vers[1].split(r'\.')
                        other_vers_major = other_and_vers[1].split(r'\.')
                        try:
                            if int(self_vers_major[0]) > int(other_vers_major[0]):
                                return False
                        except ValueError:
                            return False
