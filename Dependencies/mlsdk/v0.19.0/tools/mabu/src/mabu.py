...
from printers import print_tools, print_components, print_options, print_configs, print_resolved_projects, \
    ...
    print_output_directories, print_layout_directories, print_package_outputs, print_signing_data
...
""".format(NAME))
    ...
    print("Available segments:\n" +
          textwrap.indent(dump_build_objects(), prefix='    '))
...
def main():
    ...
    if opts.print_invoke_commands or opts.invoke:
        ...
        if err:
            diags.error(err, die=False)
        # Print the commands, then exit.
        invoke_file_paths = print_invoke_commands(opts)
        if not opts.invoke:
            sys.exit(0)
