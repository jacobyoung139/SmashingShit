...
from config import DEFAULT_PACKAGE_BINARY_DIR, BUILT_PACKAGE_EXT
...
def layout_or_package_project(opts, project):
    ...
    mlcert = _ensure_mlcert(project, opts) if make_mpk else None
    ...
    if not opts.no_layout:
        ...
        _, data_mapping, errors = project.get_package_mappings(opts.create_package, 0)
        ...
        if opts.create_package:
            ...
            data_mapping = [(src, dest) for (src, dest) in data_mapping
                            if dest.startswith("./" + DEFAULT_PACKAGE_BINARY_DIR + "/")
                            or dest.startswith(DEFAULT_PACKAGE_BINARY_DIR + "/")]

        arranger = DataLayoutArranger(data_mapping, opts.verbose, pkg_layout_path, cache_dir,
    ...
    # fetch file mappings
    build_mapping, data_mapping, data_errors = project.get_package_mappings(opts.create_package, 0)

    # validate manifest against this
    manifest_warnings, manifest_errors = _validate_manifest(opts, project, pkgflags, build_mapping, data_mapping)
    mf_path = user_relpath(project.manifest.path)

    for warning in manifest_warnings:
        diags.warn(warning, mf_path)

    errors = data_errors + manifest_errors
    for error in errors:
    ...
    # then package!
    packager = Packager(pkgflags, build_mapping, data_mapping,
...
def clean_package_project(opts, project):
    ...
    _, data_mapping, _ = project.get_package_mappings(opts.create_package, 0)
    ...
    build_mapping, data_mapping, _ = project.get_package_mappings(opts.create_package, 0)
    packager = Packager(pkgflags, build_mapping, data_mapping,
                        package_path, cache_dir)
    packager.clean()


def _validate_manifest(opts, project, pkgflags, build_mapping, data_mapping):
    """
    ...
    all_remotes = [remote.replace('\\', '/') for (local, remote) in build_mapping + data_mapping if remote]

    for component in mf.components:
        if not component.name:
            warnings.append("<component> missing a ml:name attribute")
        if not component.binary_file:
            warnings.append("<component ml:name=\"{0}\"> missing the ml:binary_name attribute".format(component.name))
        else:
            ...
            if not bin_found:
                warnings.append(
                    "<component ml:name=\"{}\"> has ml:binary_name \"{}\" "
                    "which is not bundled in this package; these are the package's files (see --print-outputs):\n\t{}"
    ...
    if mlcert:
        ...
        lines = ["Missing a developer certificate ('-s ...')."] + \
                textwrap.wrap(
                    ...
                    "For more information, see https://creator.magicleap.com/learn/guides/application-signing.", width=78)
        diags.error("\n".join(lines))
