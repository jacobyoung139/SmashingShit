    ...
    def test_launch(self):
        ...
        try:
            ...
            for i in range(0, 3):
                ...
                # stuff we care about is at the end. There we should see the result
                # of continuing to the breakpoint at main we set. Verify that
                #
                # The last three lines should be something like this:
                #    Breakpoint 1, main (argc=1, argv=0x7ffffffca8)
                #        at /home/jcortell/git/test_sdk/layout/projects/examples/native/KaliSamples/k_planar/main.cpp:4
                #    4      PlanarTest app;
                #
                # Crazy thing, though. Depending on the size of the console, the breakpoint output
                # line can come across split across two lines or as one long line!
                #

                lines = output.split('\n')
                self.assertTrue(len(lines) > 6, output)
                last_index = -1
                if lines[last_index] == '':  # -1 is a trailing newline
                    last_index -= 1
                if "Remote connection closed" in lines[last_index]:
                    # Also skip a "Remote connection closed" line if present.
                    last_index -= 1

                line1 = lines[last_index - 2]
                line2 = lines[last_index - 1]
                line3 = lines[last_index]
                if line2.startswith("Breakpoint"):
                    # one line line
                    self.assertTrue(re.search("^Breakpoint 1, main \(.*[/\\\\]main\.cpp:[\d]+", line2) is not None, output)
                    self.assertTrue(re.search("MLLifecycleCallbacks callbacks{};", line3) is not None, output)
                else:
                    # split
                    self.assertTrue(re.search("^Breakpoint 1, main \(", line1) is not None, ("line1: %s, full output:\n%s" % (line1, output)))
                    self.assertTrue(re.search("^ *at .*[/\\\\]main\.cpp:", line2) is not None, output)
                    self.assertTrue(re.search("MLLifecycleCallbacks callbacks{};", line3) is not None, output)
...
int main() {
    ...
    MLLifecycleInit(&callbacks, nullptr);
    for (int loop = 0; ;loop++) {
...
'''
    ...
    def package_mldb_tests(self, device, test_package_name):
        ...
        device.ls("/documents", wide=False, directory=True, package=test_package_name)
        test_file = __file__
        device.push(test_file, "/documents/push_test_file", package=test_package_name)
        with tempfile.TemporaryDirectory() as tmpdir:
            device.pull("/documents/push_test_file", tmpdir, package=test_package_name)
