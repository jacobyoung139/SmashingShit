...
_mldb() {
    ...
    local gen_commands="bugreport connect devices disconnect forward get-state get-serialno help kill-server reboot reverse start-server tcpip usb version"
    local ml_commands="attach audiocapture audioplayer capture crashdump getprop install launch log packages perf ps reset-headpose setprop shutdown terminate uninstall"
    local oobe_commands="access-status lock unlock changepin"
    local sync_commands="cp ls mkdir mv pull push rm"
    local connection_commands="controller ifconfig mlma wifi"
    local public_commands="${gen_commands} ${ml_commands} ${oobe_commands} ${sync_commands} ${connection_commands}"
...
_mldb_cmd_controller() {
    ...
    local cur="${COMP_WORDS[COMP_CWORD]}"
    if [[ $i -eq $COMP_CWORD ]]; then
        COMPREPLY=( $(compgen -W "pairing status info accept forget" -- "$cur") )
    elif [[ $((i+1)) -eq $COMP_CWORD && "${COMP_WORDS[i]}" == "pairing" ]]; then
