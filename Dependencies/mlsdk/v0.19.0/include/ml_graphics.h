...
typedef enum MLGraphicsFlags {
  ...
  */
  ...
  /*!
    ...
    \deprecated The client must ensure correct order in its final output buffer.
  */
...
typedef enum MLGraphicsProjectionType {
  ...
  /*!
    ...
    \deprecated Clients should use MLGraphicsProjectionType_SignedZ
  */
...
/*!
  ...
  \deprecated See MLGraphicsFrameParamsEx for replacement
*/
...
/*!
  ...
  #MLGraphicsBeginFrameEx from the client to defined the requested
  characteristics of the next rendered frame.
...
/*!
  ...
  \param frame_params The object to initialize with default settings.
  \apilevel 3
  \priv none
*/
...
typedef struct MLGraphicsFrameInfo {
  ...
  MLGraphicsVirtualCameraInfoArray virtual_camera_info_array;
} MLGraphicsFrameInfo;

ML_STATIC_INLINE void MLGraphicsFrameInfoInit(MLGraphicsFrameInfo *frame_info) {
  frame_info->version = 1;
...
/*!
  ...
  \param filename Absolute path to a file to use for caching. Location must be writeable by application.
  \param max_entry_bytes Reserved for future use. Maximum size for a single blob entry. Currently defaults to 512KB
  \param max_total_bytes Reserved for future use. Maximum total file size for the blob. Currently defaults to 8MB
...
/*!
  ...
  \param out_graphics_client The handle to the created graphics client. Only valid if result is MLResult_Ok.
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsCreateClientGL(const MLGraphicsOptions *options, const MLHandle opengl_context, MLHandle *out_graphics_client);
...
/*!
  ...
  \param options The graphics options for resource creation.
  \param vulkan_instance The Vulkan instance used to create the physical and logical device.
...
/*!
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsSetFrameTimingHint(const MLHandle graphics_client, MLGraphicsFrameTimingHint hint);
...
/*!
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsInitFrameParams(MLGraphicsFrameParams *data);
...
/*!
  ...
  \param out_handle A reference handle for this frame to be provided at end of frame. Only valid if result is MLResult_Ok.
  ...
  \deprecated See MLGraphicsBeginFrameEx for replacement
*/
ML_API MLResult ML_CALL MLGraphicsBeginFrame(const MLHandle graphics_client, const MLGraphicsFrameParams *params, MLHandle *out_handle, MLGraphicsVirtualCameraInfoArray *out_array);
...
/*!
  ...
  \param graphics_client The graphics client to query.
  \param params The frame parameters for the frame.
  \param out_frame_info Information returned for use when rendering the frame. Only valid if result is MLResult_Ok.
  \retval MLResult_Ok Frame was started successfully.
  \retval MLResult_InvalidParam Frame was not started due to an invalid parameter.
  \retval MLResult_Timeout Blocking timeout limit reached.
  \retval MLResult_UnspecifiedFailure Frame was not started due to an unknown error.
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsBeginFrameEx(const MLHandle graphics_client, const MLGraphicsFrameParamsEx *params, MLGraphicsFrameInfo *out_frame_info);
...
/*!
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsSignalSyncObjectGL(const MLHandle graphics_client, const MLHandle sync_object);
...
/*!
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsGetClipExtents(const MLHandle graphics_client, MLGraphicsClipExtentsInfoArray *out_array);
...
/*!
  ...
  \param graphics_client The graphics client to query.
  \param out_targets The render target info array. Only valid if result is MLResult_Ok.
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsGetRenderTargets(const MLHandle graphics_client, MLGraphicsRenderTargetsInfo *out_targets);
...
/*!
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsEndFrame(const MLHandle graphics_client, MLHandle handle);
...
/*!
  ...
  \param out_info Current performance information for the client. Only valid if result is MLResult_Ok.
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsGetClientPerformanceInfo(const MLHandle graphics_client, MLGraphicsClientPerformanceInfo *out_info);
