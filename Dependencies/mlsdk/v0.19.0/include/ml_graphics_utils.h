...
/*!
  ...
  \param out_required_extension_properties Either NULL or a pointer to an array of VkExtensionProperties.
  \param out_extension_property_count Is a pointer to an integer related to the number of extensions required or queried.
...
/*!
  ...
  \param media_handle The media api handle referring to the surface to import.
  \param out_imported_surface_info A struct containing the resulting resource handles and information for sampler creation.
  ...
  \priv none
*/
ML_API MLResult ML_CALL MLGraphicsImportVkImageFromMediaHandle(const VkDevice vulkan_logical_device, const uint64_t media_handle, MLGraphicsImportedMediaSurface *out_imported_surface_info);
#endif
...
/*!
  ...
  \param info: The per virtual camera info for rendering depth.
  \param meshes: The meshes for which the depth info needs to be populated
...
*/
ML_API MLResult ML_CALL MLMeshingPopulateDepth(const MLHandle client_handle, const MLMeshingDepthBufferInfo *info, const MLMeshingMesh *meshes);
